...
-- Allow anonymous tracking (for non-authenticated users)
...
-- Allow authenticated users to track their own data
CREATE POLICY "Users can track their own analytics" 
ON public.user_analytics 
FOR INSERT 
USING (auth.uid() = user_id OR auth.uid() IS NULL);

-- Admin access policy (you can modify this based on your admin setup)
